...
export class GanttChartV2Component implements OnInit {
    ...
    constructor(public route: ActivatedRoute, private services: ApiService, private mqttService: MqttService, private router: Router,
    ...
    ngOnDestroy() {
        ...
        _.each(this.clear, function (obj:any) {
            clearInterval(obj)
        });
    }
    ...
    listenSocket() {
        ...
        this.mqttService.observe(taskComplete, { qos: 1 }).subscribe((message: IMqttMessage) => {
            ...
            console.log('Task_complete topic subscribe', object)
            if (this.flightDet.flightSchedulesId == object.data.flightSchedulesId && object.data.taskActualEndTime != null) {
                this.taskCounts.taskCompletedCount = object.data.taskCompletedCount;
            }
            if (this.flightDet.flightSchedulesId == object.data.flightSchedulesId) {
                for (let index = 0; index < this.taskResp.length; index++) {
                    const element = this.taskResp[index];
                    if (element.resourceMappingId == object.data.resourceMappingId && object.data.taskActualStartTime != null && object.data.taskActualEndTime != null) {
                        console.log("event time change")
                        element.taskActualEndTime = object.data.taskActualEndTime;
                        element.taskActualStartTime = object.data.taskActualStartTime;
                        if (this.currFilter != 'All') {
                            this.filterTask(this.currFilter)
                        }
                    }

                    if (element.resourceMappingId == object.data.resourceMappingId && object.data.taskActualStartTime != null && object.data.taskActualEndTime == null) {
                        element.taskActualStartTime = object.data.taskActualStartTime;
                        console.log("inprogress time change")
                        if (this.currFilter != 'All') {
                            this.filterTask(this.currFilter)
                        }
                    }
                    ...
                    if (object.data.taskName == "Chocks on") {
                        element.actualArrivalTime = object.data.taskActualEndTime
                    }
            ...
            if (this.flightDet.flightScheduleId == object.data.flightScheduleId) {
                if (object.data.taskName == "Chocks on") {
    ...
    convertTimeFromValue(param, value, actualEnd, scheduleType, Timing, sta, ts): any {
        ...
        if (scheduleType == 0) {
            // if(ts.flightType !== 0 && ts.actualArrivalTime !== null){
    ...
    createGroups() {
        ...
        for (var index = 0; index < this.Assignedtasks.length; index++) {
            ...
            contentdivs = '<div fxLayout="row">' +
                '<div fxFlex class="userName">' +
                '<div>' + ts.name + '</div>' +
                '</div>'
            contentdiv = '<div fxLayout="row">' +
                '<div fxFlex class="cls2">' +
                '<div>' + ts.name + '</div>' +
                '</div>'
            let ethold, sthold;
            ...
            leftcontent = '<div class="i-gantt-details-' + classname + '">' +
                '<div class="activities-title">' + ts.taskName + '<span class="task-time pull-right task-' + classname + '">' + this.appUrl.getHrsmintues(pstime, this.currentStation.timezone) + ' - ' + this.appUrl.getHrsmintues(petime, this.currentStation.timezone) + '</span></div>' +
                '</div>'
            // HTML content with color logic for event tasks
            if (mz(stime).isSame(etime)) {
                contentdivs = '<div fxLayout="row">' + '<div fxFlex class="diamond-' + classname + '"><span class="userName-event">' + ts.name + '</span></div>' +
                    '</div>',
                    leftcontent = '<div  class="i-gantt-details-' + classname + '">' +
                    '<div class="activities-title">' + ts.taskName + '<span class="task-time-event pull-right task-' + classname + '">' + this.appUrl.getHrsmintues(petime, this.currentStation.timezone) + '</span></div>' +
                    '</div>'
            }
            if (mz(startTimes).isSame(endTimes)) {
                contentdiv = '<div fxLayout="row">' + '<div fxFlex class="diamond-' + classnames + '"><span class="userName-event">' + '</span></div>' +
                    '</div>'
            }
            ...
            if (stime && ts.taskName == "Chocks on") {
                this.start1 = mz(stime).tz(this.currentStation.timezone).toDate();
            }
            if (etime && ts.taskName == "Chocks off") {
                this.end1 = mz(this.currEnd).tz(this.currentStation.timezone).toDate();
            }
    ...
    filterTask(value) {
        ...
        if (value == 'All') {
            let filteredContent = this.taskResp.filter(values => values.flightSchedulesId);
            console.log(filteredContent)
            this.Assignedtasks = [];
            this.Assignedtasks = filteredContent;
            this.clearChart();
        } else if (value == 'yts') {
            let filteredContent = this.taskResp.filter(values => values.taskActualStartTime == null);
            this.Assignedtasks = [];
            this.Assignedtasks = filteredContent;
            this.clearChart();
        }
        else if (value == 'inprogress') {
            let filteredContent = this.taskResp.filter(values => values.taskActualStartTime != null && values.taskActualEndTime == null);
            this.Assignedtasks = [];
            this.Assignedtasks = filteredContent;
            this.clearChart();
    ...
    createTimeline() {
        ...
        if (_self.start1 != undefined) {
            _self.timeline.addCustomTime(_self.start1);
        }

        if (_self.end1 != undefined) {
                    ...
                    if (v.tRef == "inprogress" && v.actualtask == "actual") {
                        ...
                        for (m in _self.timeline.groupsData._data) {
                            ...
                            if (left.tRef == "inprogress") {
                                ...
                                let tend = left.rend;
                                //Applying left side content color logic for the respective rows accordingly
                                if (left.taskName == v.details.taskName) {
                                    left.className = v.className;
                                }
                                left.content = '<div class="i-gantt-details-' + left.className + '">' +
                                    '<div class="activities-title">' + tname + '<span class="task-time pull-right task-' + left.className + '">' + _self.appUrl.getHrsmintues(tstart, _self.currentStation.timezone) + ' - ' + _self.appUrl.getHrsmintues(tend, _self.currentStation.timezone) + '</span></div>' +
                                    '</div>'
                            ...
                            var id = setInterval(function () {
                                ...
                                for (p in _self.timeline.groupsData._data) {
                                    ...
                                    if (left.resId == object.data.resourceMappingId) {
                                        ...
                                        //for left side content end time update
                                        if (!left.rend) {
                                            left.rend = null;
                                        }
                                        //Applying left side content color logic for the respective rows accordingly
                                        if (left.taskName == value.details.taskName) {
                                            left.className = value.className;
                                        }
                                        left.content = '<div class="i-gantt-details-' + left.className + '">' +
                                            '<div class="activities-title">' + tname + '<span class="task-time pull-right task-' + left.className + '">' + _self.appUrl.getHrsmintues(left.rstart, _self.currentStation.timezone) + ' - ' + _self.appUrl.getHrsmintues(left.rend, _self.currentStation.timezone) + '</span></div>' +
                                            '</div>'
                                        _self.timeline.groupsData.update(left);
                                    }
                                }
                                ...
                                if (left.resId == object.data.resourceMappingId) {
                                    ...
                                    if (mz(left.rstart).isSame(left.rend)) {
                                        left.content = '<div class="i-gantt-details-' + value.className + '">' +
                                            '<div class="activities-title">' + object.data.taskName + '<span class="task-time-event pull-right task-' + value.className + '">' + _self.appUrl.getHrsmintues(left.rend, _self.currentStation.timezone) + '</span></div>' +
                                            '</div>'
                                    } else {
                                        left.content = '<div class="i-gantt-details-' + value.className + '">' +
                                            '<div class="activities-title">' + object.data.taskName + '<span class="task-time pull-right task-' + value.className + '">' + _self.appUrl.getHrsmintues(left.rstart, _self.currentStation.timezone) + ' - ' + _self.appUrl.getHrsmintues(left.rend, _self.currentStation.timezone) + '</span></div>' +
                                            '</div>'
                                    }
                                    _self.timeline.groupsData.update(left);
                                }
                            ...
                            if (value.tRef == "inprogress" && value.actualtask == "actual") {
                                ...
                                for (p in _self.timeline.groupsData._data) {
                                    ...
                                    if (left.tRef == "inprogress") {
                                        ...
                                        left.content = '<div class="i-gantt-details-' + left.className + '">' +
                                            '<div class="activities-title">' + left.taskName + '<span class="task-time pull-right task-' + left.className + '">' + _self.appUrl.getHrsmintues(left.rstart, _self.currentStation.timezone) + ' - ' + _self.appUrl.getHrsmintues(left.rend, _self.currentStation.timezone) + '</span></div>' +
                                            '</div>'
                                        _self.timeline.groupsData.update(left);
                                    }
                                }
    ...
    // function to navigate to gantt chart matrix
    toMatrix() {
        this.router.navigate(['/station/flifo/matrix']);
    }
